<div align="center">

<a href="https://damiandania.com">
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/damiandania/damiandania/main/assets/dania-mark-white.svg" />
    <img src="https://raw.githubusercontent.com/damiandania/damiandania/main/assets/dania-mark-ink.svg" width="72" alt="Damian Dania" />
  </picture>
</a>

# Hi, I'm Damian 👋

##### FULLSTACK ENGINEER &nbsp;·&nbsp; PARIS, FRANCE 🇫🇷

<a href="https://readme-typing-svg.demolab.com">
  <img src="https://readme-typing-svg.demolab.com?font=JetBrains+Mono&weight=600&size=20&pause=1200&color=6366F1&center=true&vCenter=true&width=640&height=40&lines=Ship+Fast%2C+Scale+Smart%2C+Deploy+Often.;42+Paris+graduate+%C2%B7+Architecture+%26+security+first;Interfaces+that+delight%2C+systems+that+never+break." alt="Ship Fast, Scale Smart, Deploy Often" />
</a>

<p>
  <a href="https://damiandania.com"><img src="https://img.shields.io/badge/Portfolio-0A0A0A?style=for-the-badge&logo=vercel&logoColor=white" alt="Portfolio" /></a>
  <a href="https://www.linkedin.com/in/damiandania/"><img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn" /></a>
  <a href="mailto:damiandania@gmail.com"><img src="https://img.shields.io/badge/Email-EA4335?style=for-the-badge&logo=gmail&logoColor=white" alt="Email" /></a>
</p>

<img src="https://img.shields.io/badge/%E2%97%8F%20Available%20for%20work-15803D?style=flat-square&labelColor=15803D" alt="Available for work" />

</div>

---

## 🧭 About

I'm a **42 Paris (École 42)** graduate and a Fullstack Engineer based in Paris. I like taking a blank repo all the way to a real product in production — and I care as much about the **architecture** behind it as the **interface** in front of it.

In my first year shipping professionally I've built a wide range of products: cultural platforms like *Nemausus* (3D encyclopedias and interactive scavenger hunts), multi-tenant SaaS, and CRM / back-office systems handling cross-branch operations. I work **architecture- and security-first**, on foundations built around **OWASP best practices**, and I lean on AI tooling to ship faster without cutting corners.

```yaml
role:      Fullstack Engineer
location:  Paris, France · available worldwide
focus:     Architecture · Web security · UI/UX · Systems that scale
principles:
  - Security first     # OWASP-aware decisions baked in, not bolted on
  - Performance        # Lighthouse 100 is the floor, not the goal
  - UI craftsmanship   # Every shadow, radius and easing chosen with intent
```

---

## 🛠️ Tech Stack

<div align="center">
<sub><b>LANGUAGES</b></sub>
<table>
  <tr>
    <td align="center" width="88"><img src="https://api.iconify.design/simple-icons/typescript.svg?color=%233178C6&width=44&height=44" width="44" height="44" alt="TypeScript" /><br/><sub>TypeScript</sub></td>
    <td align="center" width="88"><img src="https://api.iconify.design/simple-icons/javascript.svg?color=%23F7DF1E&width=44&height=44" width="44" height="44" alt="JavaScript" /><br/><sub>JavaScript</sub></td>
    <td align="center" width="88"><img src="https://api.iconify.design/simple-icons/python.svg?color=%233776AB&width=44&height=44" width="44" height="44" alt="Python" /><br/><sub>Python</sub></td>
    <td align="center" width="88"><img src="https://api.iconify.design/simple-icons/swift.svg?color=%23F05138&width=44&height=44" width="44" height="44" alt="Swift" /><br/><sub>Swift</sub></td>
    <td align="center" width="88"><img src="https://api.iconify.design/simple-icons/php.svg?color=%23777BB4&width=44&height=44" width="44" height="44" alt="PHP" /><br/><sub>PHP</sub></td>
    <td align="center" width="88"><img src="https://api.iconify.design/simple-icons/c.svg?color=%2300599C&width=44&height=44" width="44" height="44" alt="C" /><br/><sub>C</sub></td>
    <td align="center" width="88"><img src="https://api.iconify.design/simple-icons/cplusplus.svg?color=%2300599C&width=44&height=44" width="44" height="44" alt="C++" /><br/><sub>C++</sub></td>
  </tr>
</table>

<sub><b>FRONTEND</b></sub>
<table>
  <tr>
    <td align="center" width="88"><img src="https://api.iconify.design/simple-icons/vuedotjs.svg?color=%2342D392&width=44&height=44" width="44" height="44" alt="Vue" /><br/><sub>Vue</sub></td>
    <td align="center" width="88"><img src="https://api.iconify.design/simple-icons/nuxtdotjs.svg?color=%2300DC82&width=44&height=44" width="44" height="44" alt="Nuxt" /><br/><sub>Nuxt</sub></td>
    <td align="center" width="88"><img src="https://api.iconify.design/simple-icons/react.svg?color=%2361DAFB&width=44&height=44" width="44" height="44" alt="React" /><br/><sub>React</sub></td>
    <td align="center" width="88"><img src="https://api.iconify.design/simple-icons/astro.svg?color=%23FF5D01&width=44&height=44" width="44" height="44" alt="Astro" /><br/><sub>Astro</sub></td>
    <td align="center" width="88"><img src="https://api.iconify.design/simple-icons/tailwindcss.svg?color=%2338BDF8&width=44&height=44" width="44" height="44" alt="Tailwind CSS" /><br/><sub>Tailwind</sub></td>
  </tr>
</table>

<sub><b>BACKEND · DATA</b></sub>
<table>
  <tr>
    <td align="center" width="88"><img src="https://api.iconify.design/simple-icons/nodedotjs.svg?color=%235FA04E&width=44&height=44" width="44" height="44" alt="Node.js" /><br/><sub>Node.js</sub></td>
    <td align="center" width="88">
      <picture>
        <source media="(prefers-color-scheme: dark)" srcset="https://api.iconify.design/simple-icons/django.svg?color=%2344B78B&width=44&height=44" />
        <img src="https://api.iconify.design/simple-icons/django.svg?color=%23092E20&width=44&height=44" width="44" height="44" alt="Django" />
      </picture><br/><sub>Django</sub>
    </td>
    <td align="center" width="88"><img src="https://api.iconify.design/simple-icons/supabase.svg?color=%233ECF8E&width=44&height=44" width="44" height="44" alt="Supabase" /><br/><sub>Supabase</sub></td>
    <td align="center" width="88"><img src="https://api.iconify.design/simple-icons/postgresql.svg?color=%23336791&width=44&height=44" width="44" height="44" alt="PostgreSQL" /><br/><sub>PostgreSQL</sub></td>
    <td align="center" width="88"><img src="https://api.iconify.design/simple-icons/mysql.svg?color=%2300618A&width=44&height=44" width="44" height="44" alt="MySQL" /><br/><sub>MySQL</sub></td>
  </tr>
</table>

<sub><b>DEVOPS · AI</b></sub>
<table>
  <tr>
    <td align="center" width="88"><img src="https://api.iconify.design/simple-icons/docker.svg?color=%232496ED&width=44&height=44" width="44" height="44" alt="Docker" /><br/><sub>Docker</sub></td>
    <td align="center" width="88"><img src="https://api.iconify.design/simple-icons/nginx.svg?color=%23009639&width=44&height=44" width="44" height="44" alt="Nginx" /><br/><sub>Nginx</sub></td>
    <td align="center" width="88">
      <picture>
        <source media="(prefers-color-scheme: dark)" srcset="https://api.iconify.design/simple-icons/vercel.svg?color=%23FFFFFF&width=44&height=44" />
        <img src="https://api.iconify.design/simple-icons/vercel.svg?color=%23000000&width=44&height=44" width="44" height="44" alt="Vercel" />
      </picture><br/><sub>Vercel</sub>
    </td>
    <td align="center" width="88"><img src="https://api.iconify.design/simple-icons/git.svg?color=%23F05032&width=44&height=44" width="44" height="44" alt="Git" /><br/><sub>Git</sub></td>
    <td align="center" width="88"><img src="https://api.iconify.design/simple-icons/claude.svg?color=%23D97757&width=44&height=44" width="44" height="44" alt="Claude" /><br/><sub>Claude</sub></td>
    <td align="center" width="88">
      <picture>
        <source media="(prefers-color-scheme: dark)" srcset="https://api.iconify.design/simple-icons/openai.svg?color=%23FFFFFF&width=44&height=44" />
        <img src="https://api.iconify.design/simple-icons/openai.svg?color=%23000000&width=44&height=44" width="44" height="44" alt="GPT" />
      </picture><br/><sub>GPT</sub>
    </td>
    <td align="center" width="88"><img src="https://api.iconify.design/simple-icons/googlegemini.svg?color=%230C98F0&width=44&height=44" width="44" height="44" alt="Gemini" /><br/><sub>Gemini</sub></td>
  </tr>
</table>
</div>

---

## 🚀 Featured Work

> A selection of architecture-first builds — SaaS platforms, dashboards and full-stack experiences.
> 🟢 Live · 🔒 Private · 📖 Open source · 🚧 In progress

| Project | What it is | Stack |
|---|---|---|
| **[Nemausus](https://nemausus-rouge.vercel.app)** 🟢 | Interactive 3D heritage platform for the monuments of Nîmes — AR-ready models, a 3D encyclopedia, multilingual content and a gamified scavenger hunt. | `Nuxt` `Vue` `TS` `Supabase` `GSAP` |
| **Nemausus Dashboard** 🔒 | Analytics & admin console for the Nemausus experience — visits, audio tours, scavenger hunts and QR scanning. | `Nuxt` `Vue` `TS` `Supabase` `PostgreSQL` |
| **MiddleSpace** 🚧 | Client-file micro-SaaS: secure uploads, rule-based media processing (sharp + ffmpeg) and multi-org storage with RLS, MFA/passkeys and an AI agent layer. | `Nuxt` `Vue` `TS` `Supabase` `Claude` |
| **[Dev Monitor](https://github.com/damiandania/DevMonitor)** 📖 | Native macOS supervisor that launches, watches and auto-recycles your JS/TS dev servers — live CPU/memory graphs, crash revival and AI diagnostics. | `Swift` `SwiftUI` `Claude` |
| **Insight Core** 🚧 | RCS activation platform with a Vue Flow visual scenario editor, contact-list campaigns, conversation tracking and a Claude assistant that co-builds scenarios. | `Nuxt` `Vue` `TS` `Supabase` `Twilio` |
| **RYM Platform** 🔒 | B2B marketing-ops SaaS: media packs, custom campaign "levers", POS material orders and Sellsy-integrated invoicing. | `Nuxt` `Vue` `TS` `Supabase` `Resend` |
| **Devops Console** 🔒 | Observability console: ingests Vercel logs, visual analytics and AI-synthesized daily incident reports ranked by severity. | `Nuxt` `Vue` `Supabase` `Claude` `Gemini` |
| **Yuzu** 🚧 | Restaurant back-office: recipe costing, inventory, suppliers, staff and granular role-based permissions. | `Nuxt` `Vue` `TS` `Supabase` |
| **[Camagru](https://github.com/damiandania/Camagru)** 📖 | Photo social network with email auth, webcam capture, filter editing, a public feed, likes and comments — custom MVC. | `PHP` `JS` `MySQL` `Docker` `Nginx` |
| **[Transcendence](https://github.com/damiandania/Transcendence)** 📖 | Pusheen-themed web arcade — real-time Pong & Space Invaders with 42 OAuth, stats dashboards and global leaderboards. | `Django` `PostgreSQL` `Docker` `Nginx` |

<div align="center">

**[→ See the full portfolio](https://damiandania.com)**

</div>

---

<details>
<summary><b>🎓 42 Curriculum — low-level foundations (C / C++)</b></summary>

<br/>

<div align="center">
  <table>
    <tr>
      <td align="center">
        <a href="https://github.com/damiandania/Libft">
          <img src="https://raw.githubusercontent.com/damiandania/damiandania/main/Pics/Libft.png" alt="Libft" width="140" height="140"><br/>
          <b>Libft</b>
        </a><br/>
        <sub>Custom C standard library</sub>
      </td>
      <td align="center">
        <a href="https://github.com/damiandania/Printf">
          <img src="https://raw.githubusercontent.com/damiandania/damiandania/main/Pics/Printf.png" alt="ft_printf" width="140" height="140"><br/>
          <b>ft_printf</b>
        </a><br/>
        <sub>printf reimplementation</sub>
      </td>
      <td align="center">
        <a href="https://github.com/damiandania/Get_next_line">
          <img src="https://raw.githubusercontent.com/damiandania/damiandania/main/Pics/Get_next_line.png" alt="Get_next_line" width="140" height="140"><br/>
          <b>Get_next_line</b>
        </a><br/>
        <sub>Read a line from an fd</sub>
      </td>
    </tr>
    <tr>
      <td align="center">
        <a href="https://github.com/damiandania/Push_swap">
          <img src="https://raw.githubusercontent.com/damiandania/damiandania/main/Pics/Push_swap.png" alt="Push_swap" width="140" height="140"><br/>
          <b>Push_swap</b>
        </a><br/>
        <sub>Sorting algorithm</sub>
      </td>
      <td align="center">
        <a href="https://github.com/damiandania/pipex">
          <img src="https://raw.githubusercontent.com/damiandania/damiandania/main/Pics/Pipex.png" alt="Pipex" width="140" height="140"><br/>
          <b>Pipex</b>
        </a><br/>
        <sub>Unix pipeline recreation</sub>
      </td>
      <td align="center">
        <a href="https://github.com/damiandania/So_long">
          <img src="https://raw.githubusercontent.com/damiandania/damiandania/main/Pics/So_long.png" alt="So_long" width="140" height="140"><br/>
          <b>So_long</b>
        </a><br/>
        <sub>2D game in C</sub>
      </td>
    </tr>
    <tr>
      <td align="center">
        <a href="https://github.com/damiandania/Philosophers">
          <img src="https://raw.githubusercontent.com/damiandania/damiandania/main/Pics/Philosophers.png" alt="Philosophers" width="140" height="140"><br/>
          <b>Philosophers</b>
        </a><br/>
        <sub>Threads & mutexes</sub>
      </td>
      <td align="center">
        <a href="https://github.com/damiandania/Minishell">
          <img src="https://raw.githubusercontent.com/damiandania/damiandania/main/Pics/Minishell.png" alt="Minishell" width="140" height="140"><br/>
          <b>Minishell</b>
        </a><br/>
        <sub>A shell in C</sub>
      </td>
      <td align="center">
        <a href="https://github.com/damiandania/Cub3d">
          <img src="https://raw.githubusercontent.com/damiandania/damiandania/main/Pics/Cub3d.png" alt="Cub3d" width="140" height="140"><br/>
          <b>Cub3d</b>
        </a><br/>
        <sub>Raycasting engine</sub>
      </td>
    </tr>
    <tr>
      <td align="center">
        <a href="https://github.com/damiandania/Cpp">
          <img src="https://raw.githubusercontent.com/damiandania/damiandania/main/Pics/CPP.png" alt="C++ modules" width="140" height="140"><br/>
          <b>CPP Modules</b>
        </a><br/>
        <sub>Modern C++ (0–9)</sub>
      </td>
      <td align="center">
        <a href="https://github.com/damiandania/Webserv">
          <img src="https://raw.githubusercontent.com/damiandania/damiandania/main/Pics/Webserv.png" alt="Webserv" width="140" height="140"><br/>
          <b>Webserv</b>
        </a><br/>
        <sub>HTTP server in C++</sub>
      </td>
      <td align="center">
        <a href="https://github.com/damiandania/Inception">
          <img src="https://raw.githubusercontent.com/damiandania/damiandania/main/Pics/Inception.png" alt="Inception" width="140" height="140"><br/>
          <b>Inception</b>
        </a><br/>
        <sub>Docker orchestration</sub>
      </td>
    </tr>
    <tr>
      <td align="center">
        <a href="https://github.com/damiandania/NetPractice">
          <img src="https://raw.githubusercontent.com/damiandania/damiandania/main/Pics/Netpractice.png" alt="NetPractice" width="140" height="140"><br/>
          <b>NetPractice</b>
        </a><br/>
        <sub>Networking & subnetting</sub>
      </td>
      <td align="center">
        <a href="https://github.com/damiandania/Invaders">
          <img src="https://raw.githubusercontent.com/damiandania/damiandania/main/Pics/Invaders.png" alt="Invaders" width="140" height="140"><br/>
          <b>Invaders</b>
        </a><br/>
        <sub>Arcade game in JS</sub>
      </td>
      <td align="center"></td>
    </tr>
  </table>
</div>

</details>

---

<div align="center">

*Fullstack engineer focused on architecture, security, and interfaces that feel right.*
**Based in Paris — available worldwide.**

<a href="https://damiandania.com">damiandania.com</a> · <a href="mailto:damiandania@gmail.com">damiandania@gmail.com</a>

</div>
